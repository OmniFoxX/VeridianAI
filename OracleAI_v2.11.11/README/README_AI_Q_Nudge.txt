Enablement: add the two lines, save, restart OracleAI. On boot you'll see [AIQ_NUDGE] Initialised... and [AIQ_NUDGE] enabled in config: True in the backend console.
...
You start a long run in OracleAI's normal chat UI (e.g., "do the WCAG audit")
Sage starts working
...
*open terminal* python E:\OracleAI_v2.2\tools\aiq_nudge_send.py "skip the lint pass, do the structural audit first"

The script writes the signed file and exits — back to your prompt
Within ~1 agentic step, Sage picks it up. The OracleAI chat UI shows the green confirmation banner with the preview. Her next reasoning incorporates your guidance.
Sage's response continues appearing in the OracleAI chat UI, not the terminal
...
C:\path\to\python.exe E:\OracleAI_v2.2\tools\aiq_nudge_send.py
and pipe text into it.
Tip for the WCAG audit specifically: you can also pipe a file in. If you have prep notes ready:
type wcag_guidance.txt | python E:\OracleAI_v2\tools\aiq_nudge_send.py -
That sends the entire file as one nudge — useful for longer mid-course corrections.
So: terminal = your "outbox" to Sage. Chat UI = Sage's continued response stream. They're decoupled, which is what makes it non-disruptive during long runs