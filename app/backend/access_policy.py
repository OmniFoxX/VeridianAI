...
---------------------------------------------------
    ...
    mcp_allowed     : bool  False cuts the MCP-over-HTTP channel
                            (POST /mcp/v1/jsonrpc) for this profile.

                            v2.14.2. Default TRUE, deliberately: a profile
                            should have the same abilities as any other and
                            the owner switches things OFF, rather than each
                            profile starting crippled. Modelled on
                            socials_allowed, NOT on admin_grants -- driving
                            your own assistant from your own editor is a
                            feature, not a delegated owner power.

                            One switch for the whole channel rather than a
                            per-tool checklist: the tool list grows, and a
                            checklist would drift out of date the first time
                            somebody adds one.
...
def mcp_allowed(username: str) -> bool:
    """False only when an explicit policy turns the MCP channel off.

    Fail-OPEN like socials_allowed, not fail-closed like admin_granted: this
    gates a normal feature, and a transient error reading the policy store
    should not silently disconnect somebody's editor. The API token is still
    required and still verified regardless.
    """
    try:
        return bool(get_policy(username).get("mcp_allowed", True))
    except Exception:
        return True
