...
# CACHE-BUSTS MOVED OUT -- see test_cache_busts.py.
#
# This file used to check them, and got the rule wrong twice on the way. First
# it pinned the literal 'chat.js?v=2.15.2a' (a throwaway value typed to force
# past a stale cache mid-development) -- a test defending an accident. Then it
# asserted the three modules were "bumped together", which went red the moment
# reasoning-log.js correctly stayed put because nothing in it had changed.
#
# The rule neither version found: a cache-bust means "this FILE changed", so
# the only always-correct value is the file's own content hash. That is now
# generated by _bust_cache.py and enforced across EVERY frontend reference by
# test_cache_busts.py, which also catches the case this file never could --
# the same file referenced from two places with two different busts.
#
# Left as a pointer rather than deleted: a concern that moved should say where
# it went, or the next person re-derives it here and gets it wrong a third
# time.
ok("cache-busts are enforced, in the test that owns them",
   os.path.exists(os.path.join(_HERE, "test_cache_busts.py")))
