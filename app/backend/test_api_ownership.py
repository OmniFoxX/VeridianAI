...
print("\n== the ora_ -> vai_ prefix rename does not revoke anybody ==")
# Renaming the mint prefix is a one-line change that silently 401s every key
# already sitting in someone's Continue.dev or Claude Desktop config. The old
# prefix stays acceptable; only what we MINT changes.
ok("new keys are minted vai_", auth._new_token().startswith("vai_"))
ok("ora_ is still accepted", "ora_" in auth.ACCEPTED_TOKEN_PREFIXES)
_legacy = "ora_" + "L" * 43
_legacy_store = {"tokens": [{"prefix": _legacy[:8],
                             "hash": auth._hash_token(_legacy),
                             "label": "issued before the rename",
                             "scopes": ["*"], "owner_ns": None,
                             "created": "2026-01-01T00:00:00"}]}
ok("a key issued before the rename still verifies",
   auth._verify_token(_legacy, _legacy_store) is not None)
ok("...but a forged one with the same prefix does not",
   auth._verify_token("ora_" + "X" * 43, _legacy_store) is None)
ok("an unprefixed token is still refused outright",
   auth._verify_token("sk-not-ours", _legacy_store) is None)
