...
_NS_PROCEDURAL: Dict[str, Any] = {}


def _procedural_memory(ns=None):
    """Return the ProceduralMemory store for a namespace.

    ns=None is the owner / single-user store, which is main.procedural -- the
    one instance the chat path already uses, so the two surfaces stay in sync.

    A non-owner namespace gets its own store under sage_data/users/<ns>/,
    cached here because loading the JSON on every tool call would be silly.
    It shares the SAME memory_logger: procedures are per-profile data, but the
    hash chain is the audit log and stays one continuous record. See the
    v2.14.2 note in procedural_memory.ProceduralMemory.
    ...
    """
    if ns:
        cached = _NS_PROCEDURAL.get(ns)
        if cached is not None:
            return cached
        try:
            import sage_engine as _se
            from procedural_memory import ProceduralMemory as _PM
            base = _se.user_data_dir(ns)
            if base is not None:
                _logger = None
                try:
                    import main as _m
                    _logger = getattr(_m, "memory_logger", None)
                except Exception:
                    pass
                inst = _PM(storage_dir=str(base / "procedural_memory"),
                           memory_logger=_logger, owner_ns=ns)
                _NS_PROCEDURAL[ns] = inst
                return inst
        except Exception:
            pass
        # Could not build a per-user store -- fall through to the shared one
        # rather than failing the call. Logged so it is not silent.
        print(f"[MCP] could not open procedural store for ns={ns!r}; "
              f"falling back to the shared store", flush=True)
    try:
...
def _tool_search_memory(args: Dict[str, Any], ns=None) -> Dict[str, Any]:
    q = str(args.get("query", "")).strip()
    if not q:
        return _result_text("[error] empty query", is_error=True)
    archives = _sage().search_all_archives(q, ns=ns)
    if not archives:
...
def _tool_save_file(args: Dict[str, Any], ns=None) -> Dict[str, Any]:
    ...
    if not isinstance(content, str):
        content = str(content)
    result = _sage().save_to_downloads(fname, content, ns=ns)
...
def _tool_recall(args: Dict[str, Any], ns=None) -> Dict[str, Any]:
    q = str(args.get("query", "")).strip()
    if not q:
        return _result_text("[error] empty query", is_error=True)
    pm = _procedural_memory(ns)
    # Try exact key first
...
def _tool_remember(args: Dict[str, Any], ns=None) -> Dict[str, Any]:
...
def _tool_remember_fail(args: Dict[str, Any], ns=None) -> Dict[str, Any]:
    key = str(args.get("key", "")).strip()
...
# Tools that read or write PER-PROFILE data and therefore receive the caller's
# namespace. Everything else is stateless -- search the web, run code, parse an
# expression -- and needs no namespace at all.
#
# One explicit set rather than sixteen changed signatures: it is the list to
# audit when adding a tool, and the question "does this touch a user's data?"
# is answered in one place instead of inferred from sixteen call sites.
#
# ADDING A TOOL: if it touches archives, downloads, procedural memory, chat
# history or the browser profile, put its name here. If you are unsure, put it
# here -- an unnecessary namespace argument is harmless; a missing one is a
# cross-profile leak.
_NS_TOOLS = frozenset({
    "search_memory",    # archives
    "save_file",        # downloads/
...
def call_tool(name: str, arguments: Dict[str, Any] | None = None,
              ns=None) -> Dict[str, Any]:
    """MCP `tools/call` dispatcher.

    `ns` is the caller's namespace, resolved from a VERIFIED API token by
    main.mcp_jsonrpc. None = owner / single-user shared store.
    ...
    if _customs is not None:
        ...
        if _c.verdict in ("pass", "repaired") and isinstance(_c.args, dict):
            arguments = _c.args
    # The browser profile is selected by a ContextVar that ws_chat sets per
    # turn; MCP never set it, so every browse ran in the OWNER's profile --
    # with the owner's cookies when persistence is on. Setting it here covers
    # browse / web_search / weather at one boundary rather than in three tools.
    try:
        _sage().set_browser_ns(ns)
    except Exception:
        pass
...
def handle_jsonrpc(request: Dict[str, Any], ns=None) -> Dict[str, Any] | None:
    """Process one JSON-RPC 2.0 request and return the response envelope.

    `ns` is the caller's namespace from their verified API token; it is
    threaded to call_tool so tools touching user data reach the right profile.
    ...
    try:
        ...
        if method == "tools/call":
            ...
            return _ok(call_tool(tname, targs, ns=ns))
        if method == "ping":
            return _ok({})
