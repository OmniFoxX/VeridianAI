...
# BEFORE any module resolves or reads a data path. The owner's chat, archives,
# uploads and downloads moved from the install directory into sage_data; this
# relocates whatever an existing install still has in the old place. It runs at
# most once, never overwrites, and never raises into the boot.
try:
    import state_migration as _state_migration
    _state_migration.migrate_owner_data_once()
except Exception as _sm_err:      # pragma: no cover - boot must not depend on it
    print("[STATE] owner-data move skipped: %s" % _sm_err, flush=True)
...
import secrets                  # staging ids for import; not previously needed
...
app = FastAPI(title="VeridianAI", version="2.17.0", docs_url=None, redoc_url=None)
...
async def api_burn(payload: dict, request: Request):
    ...
    # v2.14.3: destroy the profile's data key BEFORE the files.
    #
    # Order is the whole point. Key first means an interruption leaves data
    # that CANNOT BE READ; files first would leave a window where the key
    # still opens whatever survived. It also makes burn a stronger promise
    # than deleting files: without the key the contents are unreadable even
    # if they are later recovered from a disk image or an old backup.
    _key_burned = False
    if ns:
        try:
            import profile_keys as _pk
            _key_burned = _pk.destroy_for_profile(ns)
            if _key_burned:
                print(f"[BURN] destroyed the data key for profile {ns}", flush=True)
        except Exception as _ke:
            print(f"[BURN] could not destroy the profile key for {ns}: {_ke}",
                  flush=True)
...
async def api_get_browser_config(request: Request):
    try:
        import ui_prefs
        _ns = _safe_ns(_session_ns(request))
        return {"persist_cookies":
                bool(ui_prefs.get("browser_persist_cookies", False, ns=_ns))}
    except Exception:
...
async def api_set_browser_config(payload: dict, request: Request):
    ...
    # Whose browsing this is. Without ns it was one switch for the whole
    # install: turning cookie persistence on left it on for the next person
    # who signed in, and off again for nobody.
...
async def api_rotate_api_key(payload: dict, request: Request):
    ...
    try:
        ...
        # Rotation revokes every token for this profile, so their wraps go too
        # -- then the replacement gets one.
        if ns:
            try:
                import profile_keys as _pk, session as _sess
                _pk.clear_token_wraps(ns)
                _dek = _sess.get_session_dek(request.cookies.get(_AUTH_COOKIE))
                if _dek:
                    _pk.add_token_wrap(ns, _dek, token[:8], token)
            except Exception as _we:
                print(f"[KEYS] rotation: wrap maintenance failed: {_we}",
                      flush=True)
...
# --- API keys (scoped) ------------------------------------------------------
# Rotate replaces the ONE default key, which is the right shape for the common
# case: a single key, replaced if it leaks. These endpoints are for the other
# case -- issuing a narrow key for a specific integration, so a tool that only
# needs to read has no ability to write.
#
# The scope machinery has existed since v2.2 and nothing ever minted anything
# but ["*"]. Least privilege you cannot express is least privilege you do not
# have.
#
# Every one of these is scoped to the CALLER'S OWN profile. A profile manages
# its own keys and cannot see or revoke another's -- same abilities for every
# profile, isolation between them.

@app.get("/api/auth/keys")
async def api_keys_list(request: Request):
    """Metadata for the caller's keys. Never returns a hash or a raw token."""
    if not _is_local_client(request):
        return _cloak_not_found()
    try:
        ...
        return {
            "ok": True,
            "keys": _auth.list_tokens(ns),
            "presets": [{"id": k, "label": _auth.SCOPE_LABELS.get(k, k),
                         "scopes": v}
                        for k, v in _auth.SCOPE_PRESETS.items()],
        }
    except Exception as e:
        return {"ok": False, "error": _safe_detail(e, "keys list")}


@app.post("/api/auth/keys")
async def api_keys_create(payload: dict, request: Request):
    ...
    if not label:
        raise HTTPException(400, "a label is required -- an unlabelled key is "
                                 "one you cannot safely revoke later")
    try:
        import auth as _auth
        scopes = _auth.scopes_for_preset(preset)
        if scopes is None:
            # Explicitly NOT defaulting to full access on an unrecognised
            # preset. See auth.SCOPE_PRESETS.
            raise HTTPException(400, "unknown scope preset %r; expected one of %s"
                                     % (preset, sorted(_auth.SCOPE_PRESETS)))
        ns = _safe_ns(_session_ns(request))
        token = _auth.issue_token(ns, label, scopes)

        # v2.14.3: give the key its own way into this profile's data.
        #
        # An API caller has no session and no password, so without a wrap the
        # token authenticates and then cannot read anything. The raw token
        # exists exactly once -- here -- so this is the only moment it can be
        # done. Needs the caller's OWN data key, which their session holds
        # because they are signed in to create this.
        _wrapped = False
        if ns:
            try:
                import profile_keys as _pk, session as _sess
                _dek = _sess.get_session_dek(request.cookies.get(_AUTH_COOKIE))
                if _dek:
                    _wrapped = _pk.add_token_wrap(ns, _dek, token[:8], token)
            except Exception as _we:
                print(f"[KEYS] could not wrap the data key for a new API key: "
                      f"{_we}", flush=True)

        _audit_api_action(request, "auth.key_create",
                          {"profile": ns or "(owner)", "label": label,
                           "preset": preset, "scopes": scopes})
        return {
            "ok": True, "token": token, "shown_once": True,
            "label": label, "preset": preset, "scopes": scopes,
            "reads_your_data": _wrapped,
            "note": ("Copy this now. It is stored hashed and cannot be shown "
                     "again. It can do only what its scope allows."),
        }
    except HTTPException:
        raise
    except Exception as e:
        return {"ok": False, "error": _safe_detail(e, "key create")}


@app.post("/api/auth/keys/revoke")
async def api_keys_revoke(payload: dict, request: Request):
    """Revoke ONE of the caller's keys by its public prefix.

    The prefix is not secret -- it is shown in the list -- so the namespace
    check in auth.revoke_token is what actually prevents revoking someone
    else's key, not the unguessability of the identifier.
    """
    if not _is_local_client(request):
        return _cloak_not_found()
    ...
    try:
        import auth as _auth
        ns = _safe_ns(_session_ns(request))
        done = _auth.revoke_token(prefix, ns)
        if done and ns:
            try:
                import profile_keys as _pk
                _pk.drop_token_wrap(ns, prefix)
            except Exception as _we:
                print(f"[KEYS] revoked {prefix} but could not drop its data-key "
                      f"wrap: {_we}", flush=True)
        if done:
            _audit_api_action(request, "auth.key_revoke",
                              {"profile": ns or "(owner)", "prefix": prefix})
        return {"ok": bool(done),
                "error": None if done else "no such key in your profile"}
    except Exception as e:
...
async def api_export_inventory(request: Request):
    ...
    try:
        import data_export
        _ns = _safe_ns(_session_ns(request))
        inv = data_export.inventory(_ns)
        # A portable export needs a key to travel with it. The owner's is on
        # disk; a profile's is the one unlocked in this session. It used to
        # read "owner only", which stopped being true when profiles got keys
        # of their own.
        inv["can_portable"] = (_ns is None) or bool(_request_dek(request))
        return inv
    except Exception as e:
...
async def api_export_build(payload: dict, request: Request):
    ...
    try:
        ...
        result = await _run_in_threadpool(
            ...
            ns is None,          # is_owner
            # A profile's portable export carries HER key, from this session.
            # The old refusal was right when the only key on offer was the
            # app-wide one; it is not her data's jailer any more.
            _request_dek(request) if ns else None,
            (payload.get("passphrase") or None),
        )
        return result
    except Exception as e:
...
#
# data_import.py has had 72 passing tests and no way to reach it since it was
# written: no endpoint, no button. A person could take their data out and had
# no way to put it back, which makes the export a one-way door and the promise
# ("you can always take your data") only half true.
#
# TWO STEPS ON PURPOSE. The archive is staged and INSPECTED first, and nothing
# is written until a second, explicit call. Restoring is the one operation here
# that can overwrite somebody's history, so it does not happen as a side effect
# of choosing a file.
_IMPORT_STAGE_DIR = ".import_staging"
_IMPORT_MAX_BYTES = 20 * 1024 ** 3
_IMPORT_STALE_SEC = 6 * 3600


def _import_stage_dir():
    from config import DATA_DIR
    d = Path(DATA_DIR) / _IMPORT_STAGE_DIR
    d.mkdir(parents=True, exist_ok=True)
    return d


def _sweep_import_stage():
    """Drop staged uploads nobody came back for. An abandoned archive is a
    copy of someone's data sitting in a directory they never chose."""
    try:
        now = time.time()
        for f in _import_stage_dir().glob("*.zip"):
            try:
                if now - f.stat().st_mtime > _IMPORT_STALE_SEC:
                    f.unlink()
            except OSError:
                pass
    except Exception:
...
def _staged_path(stage_id: str):
    """Resolve a staging id to a path, or None. The id is matched against the
    directory listing rather than joined into a path, so nothing a caller sends
    is ever used to build a filename."""
    sid = str(stage_id or "")
    if not sid:
        return None
    for f in _import_stage_dir().glob("*.zip"):
...
def _import_target_ns(request: Request, target):
    """Which namespace an import may land in. Returns (ns, error).

    Rules, from the two ways people actually arrive here:

      "self"  -- your own store. Always allowed, and it is the fresh-install
                 case: someone moving machines is the owner of the new one,
                 so their export lands as the owner's data rather than as a
                 child account they then have to sign into.

      a name  -- someone else's profile. Owner-gated, and only into an EMPTY
                 one. Provisioning a returning colleague is a legitimate need;
                 silently overwriting a profile that already has history in it
                 is not, and no confirmation dialog makes it safe.
    """
    caller = _safe_ns(_session_ns(request))
    t = (target or "self")
    if t in ("self", "", None):
        return caller, None
    t = _safe_ns(t)
    if not t:
        return None, "that is not a valid profile name"
    if t == caller:
        return caller, None
    if not _is_owner(request):
        return None, ("Only the owner can import into another profile. Your "
                      "own data you may import freely.")
    try:
        import users as _users
        known = {(u.get("ns") or "") for u in _users.list_users()}
        if t not in known:
            return None, ("There is no profile called %r. Create the account "
                          "first, then import into it." % t)
    except Exception:
        return None, "the profile list could not be read"
    try:
        root = sage_engine.user_data_dir(t)
        if root and Path(root).exists() and any(Path(root).iterdir()):
            return None, ("Profile %r already has data. Importing into it "
                          "could overwrite that person's history, so it is "
                          "refused. Import into an empty profile." % t)
    except Exception:
        pass
    return t, None
...
async def api_import_inspect(request: Request, file: UploadFile = File(...)):
    """Stage an archive and report what is in it. Writes nothing to the store."""
    if not _is_local_client(request):
        return _cloak_not_found()
    _sweep_import_stage()
    sid = secrets.token_urlsafe(16)
    dest = _import_stage_dir() / (sid + ".zip")
    try:
        written = 0
        with open(dest, "wb") as out:
            while True:
                ...
                if written > _IMPORT_MAX_BYTES:
                    raise ValueError("archive is larger than the import limit")
                out.write(chunk)
        import data_import as _di
        info = await _run_in_threadpool(_di.inspect_archive, str(dest))
        if not info.get("ok"):
        ...
        try:
            ...
            pass
        return {"ok": False, "error": _safe_detail(e, "import inspect")}


@app.post("/api/import")
async def api_import_restore(payload: dict, request: Request):
    """Restore a staged archive. The only call here that writes anything."""
    if not _is_local_client(request):
        return _cloak_not_found()
    src = _staged_path(payload.get("stage_id"))
    if src is None:
        return {"ok": False, "error": "that upload is no longer staged -- "
                                      "choose the file again"}
    ns, err = _import_target_ns(request, payload.get("target"))
    if err:
        return {"ok": False, "error": err}
    try:
        import data_import as _di
        mode = str(payload.get("mode") or _di.MERGE).strip().lower()
        if mode not in (_di.MERGE, _di.REPLACE):
            return {"ok": False, "error": "unknown mode %r" % mode}
        dry = bool(payload.get("dry_run"))
        result = await _run_in_threadpool(
            _di.restore, str(src), ns, payload.get("sections") or None,
            mode=mode, dry_run=dry, passphrase=payload.get("passphrase") or None)
        if not dry and result.get("ok"):
            _audit_api_action(request, "data.import", {
                "profile": ns or "(owner)",
                "mode": mode,
                "sections": payload.get("sections"),
            ...
            try:
                src.unlink(missing_ok=True)   # the copy we staged, not theirs
            except OSError:
                pass
        return result
    except Exception as e:
        return {"ok": False, "error": _safe_detail(e, "import")}
...
async def api_get_ollama_notice(request: Request):
    """Whether to offer the optional-engine notice. Dismissal is permanent."""
    try:
        import ui_prefs
        _ns = _safe_ns(_session_ns(request))
        return {"dismissed": bool(ui_prefs.get("ollama_notice_dismissed",
                                               False, ns=_ns)),
...
async def api_dismiss_ollama_notice(payload: dict, request: Request):
    """Dismiss (or restore) the notice. Deliberately NOT owner-gated: it has no
    security surface, and gating it would leave a non-owner staring at a banner
    they cannot silence. Per-profile for the same reason -- one person
    dismissing it should not decide for everybody else."""
    try:
        import ui_prefs
...
def _api_dek(request: Request):
    """This request's profile data key, if it arrived on an API token.

    auth.require_scope unwraps it from the bearer token. None for UI requests
    (whose key lives on the session), for the owner, and for tokens issued
    before wraps existed -- all normal states.
    """
    try:
        return getattr(request.state, "api_dek", None)
    except Exception:
        return None


def _request_dek(request: Request):
    """The data key for this request, whichever door it came through.

    One accessor so call sites do not have to know whether they are serving a
    browser or an API client -- and so a new caller cannot accidentally handle
    only one of the two.
    """
    d = _api_dek(request)
    if d:
    ...
    try:
        import session as _session
        return _session.get_session_dek(request.cookies.get(_AUTH_COOKIE))
    except Exception:
        return None


def _hold_profile_key(token, ns, dek) -> bool:
    """Give a data key to a session AND to the at-rest layer. The only door.

    Three paths mint a session carrying a key: password login, the
    second-factor hop, and the re-issue after a password change. Two of them
    used to call session.set_session_dek directly, which is invisibly wrong --
    everything still reads, through atrest's system-key fallback, so the
    profile's own key quietly protects nothing. One function, three callers.

    session.py drops the at-rest registration when the last session for this
    profile ends, so nothing here has to remember to.
    """
    if not token or not dek:
        return False
    try:
        import session as _session
        if not _session.set_session_dek(token, dek):
            return False
        if ns:
            import atrest as _atrest
            _atrest.register_profile_key(ns, dek)
        return True
    except Exception as _e:
        print(f"[KEYS] could not hold the profile key for {ns}: {_e}", flush=True)
        return False


def _attach_profile_key(token, ns, password):
    """Unlock a profile's data key and hand it to the session.

    Silent no-op for the owner (ns=None -- no profile key by design) and for
    any profile created before per-profile keys existed. Both are normal
    states, not errors, so this never blocks a login: a user whose key cannot
    be opened still signs in, and reads through the shared-key fallback.
    """
    if not ns or not token:
        return False
    try:
        import profile_keys as _pk
        ...
        # Never fail a login over this. A profile whose key will not open is a
        # problem to surface elsewhere, not a lockout.
        print(f"[KEYS] could not attach profile key for {ns}: {_e}", flush=True)
    return False


def _migrate_profile_key_once(request, ns):
    """First unlock: bring this profile's existing files under its own key.

    Runs BEFORE the login response returns, on purpose. A background thread
    would race the user's first writes, and the honest cost -- a pause on one
    login, once -- is smaller than the confusion of data changing underneath
    an open session.

    Returns a result dict worth showing the person, or None when there is
    nothing to say. Never raises: a failed conversion leaves the files exactly
    as they were and they still read through the system-key fallback.
    """
    if not ns:
        return None
    try:
        import key_migration as _km
        if _km.is_done(ns):
            return None
        r = _km.run(ns)
        if not r.get("ran"):
            return None
        line = _km.summary(r)
        print(f"[KEYS] profile '{ns}' first unlock: {line}", flush=True)
        try:
            _audit_api_action(request, "profile.key_migration",
                              {"profile": ns, "summary": line,
                               "converted": r.get("converted", 0),
                               "failed": r.get("failed", 0),
                               "unreadable": r.get("unreadable", 0)})
        except Exception:
            pass
        return {"summary": line, "converted": r.get("converted", 0),
                "failed": r.get("failed", 0),
                "unreadable": r.get("unreadable", 0),
                "plaintext": r.get("plaintext", 0)}
    except Exception as _e:
        print(f"[KEYS] key migration for '{ns}' did not run: {_e}", flush=True)
        return None
...
async def mcp_jsonrpc(payload: dict, request: Request):
    ...
    """
    # v2.14.2: whose data does this call operate on? From the VERIFIED bearer
    # token (auth.require_scope -> request.state.api_principal), never from the
    # payload. None = owner / single-user shared store.
    _mcp_ns = _safe_ns(_session_ns(request))

    # Owner-switchable per profile. Default ON: a profile has the same
    # abilities as any other unless the owner turns something off.
    if _mcp_ns:
        try:
            import session as _session, access_policy as _ap
            _s = _session.get_session(request.cookies.get(_AUTH_COOKIE))
            _pr = _api_principal(request)
            _uname = (_s or {}).get("username") if _s else None
            if _uname is None and _pr is not None:
                import users as _users
                _uname = next((u.get("username") for u in _users.list_users()
                               if u.get("ns") == _mcp_ns), None)
            if _uname and not _ap.mcp_allowed(_uname):
                # 404, matching the owner-gate cloak convention: do not
                # confirm the surface exists to a profile denied it.
                return _cloak_not_found()
        except Exception:
            pass

    _audit_api_action(request, "api.mcp", {"profile": _mcp_ns or "(owner)"})
    ...
    if isinstance(payload, list):
        ...
        for req in payload:
            if not isinstance(req, dict):
                continue
            r = _mcp_handlers.handle_jsonrpc(req, ns=_mcp_ns)
            if r is not None:
                responses.append(r)
        return responses
    # Single
    ...
    resp = _mcp_handlers.handle_jsonrpc(payload, ns=_mcp_ns)
    if resp is None:
...
async def openai_list_models():
    # Deliberately does NOT take the Request: the model list is a system-level
    # resource (which GGUFs and Ollama models exist on this machine), not
    # per-profile data, so there is no namespace to resolve. An unused
    # parameter here would read as intent that is not there.
...
async def openai_chat_completions(payload: dict, request: Request):
    ...
    """
    # v2.14.2: whose data is this turn? _session_ns reads
    # request.state.api_principal, which auth.require_scope set from the
    # VERIFIED bearer token -- so this is the token's binding, never anything
    # the caller put in the payload. None = owner / shared store, which is
    # also every single-user install.
    _api_ns = _safe_ns(_session_ns(request))
    _audit_api_action(request, "api.chat", {"profile": _api_ns or "(owner)"})
    ...
    try:
        ...
        if not _atrest.is_encrypted(_raw):
            with open(dest, "wb") as _uf:  # nosemgrep -- same contained dest
                # The uploads dir is already per-profile (_uploads_dir_for_ns
                # above); the CONTENTS have to follow, or the file sits in
                # the right folder encrypted with the wrong key.
                _uf.write(_atrest.encrypt_bytes(
                    _raw, ns=_safe_ns(_session_ns(request))))
    except Exception:
        pass
    return _result
...
async def _generate_image_routed(prompt, downloads_dir=None,
                                 _customs_cleared=False, ns=None, **opts):
            ...
            if ok and isinstance(res, dict) and res.get("success") and res.get("data"):
                try:
                    ...
                    with open(_outp, "wb") as _f:  # nosemgrep -- _outp reduced to a basename + contained in _dl
                        _f.write(_atrest.encrypt_bytes(raw, ns=ns))
                    res["path"] = str(_outp)
                    res["via"] = "remote"
                except Exception:
                    pass
    ...
    result = await asyncio.to_thread(
        ...
        downloads_dir=str(_dl), ns=ns, **opts)
...
async def api_generate_image(payload: dict, request: Request):
    ...
    try:
        ...
        pass
    _img_ns = _safe_ns(_session_ns(request))
    return await _generate_image_routed(
        prompt, downloads_dir=str(_downloads_dir_for_ns(_img_ns)),
        ns=_img_ns, **opts)
...
async def api_auth_login(payload: dict, request: Request):
    ...
    # Held for _attach_profile_key below: login is the only moment we
    # legitimately have the plaintext, and unwrapping the data key needs it.
    _login_password = payload.get("password", "")
    ...
    if _methods:
        ...
        # Unwrap the data key NOW, while the plaintext password still exists,
        # and park it against the challenge. The session is minted at the
        # second-factor step, where the password is long gone -- without this,
        # every MFA profile would sign in and then be unable to read its own
        # data, which looks like corruption rather than a missing hop.
        if r.get("ns"):
            try:
                import profile_keys as _pk
                ...
                print(f"[KEYS] could not stage the profile key for MFA login "
                      f"({r.get('username')}): {_ke}", flush=True)
    ...
    _attach_profile_key(tok, r.get("ns"), _login_password)
    _body = {"success": True, "username": r["username"],
             "is_owner": r["is_owner"], "must_change": _must_change}
    _mig = _migrate_profile_key_once(request, r.get("ns"))
    if _mig:
        _body["key_migration"] = _mig
    resp = JSONResponse(_body)
    # Session cookie (NO max_age) so it's discarded when the browser/app closes
    # -> reopening requires a fresh sign-in. Server session still expires via TTL.
    resp.set_cookie(_AUTH_COOKIE, tok, httponly=True, samesite="lax",
                    secure=_cookie_secure(request))
...
async def api_auth_change_password(payload: dict, request: Request):
    ...
    # v2.14.3: re-wrap the data key BEFORE the password changes underneath it.
    # Opened with the CURRENT password, which was verified two lines up -- so
    # this needs nothing the user has not already proven they know.
    #
    # Order matters: if set_password succeeded and this then failed, the
    # profile would be unopenable. So unwrap first, and refuse the whole change
    # if the key cannot be opened, rather than leaving a locked-out profile.
    _dek = None
    if chk.get("ns"):
        try:
            import profile_keys as _pk
            _dek = _pk.unlock(chk["ns"], current)
        except Exception as _ke:
            raise HTTPException(
                500, "your password was NOT changed: the data key could not be "
                     "opened, and changing it now would lock you out of your "
                     "own data. Reference: %s" % _safe_detail(_ke, "key unwrap"))
    ...
    if _dek is not None:
        try:
            import profile_keys as _pk
            _pk.rewrap_password(chk["ns"], _dek, new)
        except Exception as _ke:
            # The password IS changed at this point. Say so loudly rather than
            # letting the user discover it at the next login.
            print(f"[KEYS] CRITICAL: password changed but re-wrap failed for "
                  f"{username}: {_ke}", flush=True)
            raise HTTPException(
                500, "your password was changed, but the data key could not be "
                     "re-wrapped. Change it back to the previous one to regain "
                     "access to your data. Reference: %s"
                     % _safe_detail(_ke, "key rewrap"))
    ...
    tok = _session.create_session(
        ...
        ttl=_AUTH_TTL)
    if _dek is not None:
        # Same door as login: the session AND at-rest, or the re-wrapped key
        # protects nothing until the next sign-in.
        _hold_profile_key(tok, chk.get("ns"), _dek)   # this tab keeps working
...
def _mint_session_response(user, ttl, must_change, request: Request,
    ...
    `mfa_token` carries the data key parked at the first login step across to
    the session being minted here. Omitting it is not an error -- it just means
    this profile has no key -- so the two verify endpoints pass it explicitly
    rather than relying on it being found."""
    import session as _session
    ...
    if mfa_token:
        try:
            import mfa as _mfa
            _d = _mfa.take_challenge_dek(mfa_token)
            if _d:
                _hold_profile_key(tok, user.get("ns"), _d)
        except Exception as _ke:
            print(f"[KEYS] could not attach the profile key after MFA: {_ke}",
                  flush=True)
    _body = {"success": True, "username": user.get("username"),
             "is_owner": bool(user.get("is_owner")),
             "must_change": bool(must_change)}
    _mig = _migrate_profile_key_once(request, user.get("ns"))
    if _mig:
        _body["key_migration"] = _mig
    resp = JSONResponse(_body)
    resp.set_cookie(_AUTH_COOKIE, tok, httponly=True, samesite="lax",
...
async def api_auth_users_create(request: Request, payload: dict):
    ...
    # v2.14.3: give the new profile its own data key.
    #
    # `owner_recovery` defaults TRUE, matching the shape most installs need and
    # the one that cannot lose data. FALSE means nobody -- including the owner
    # -- can ever read this profile without its password, and a forgotten
    # password is unrecoverable. The UI states both outcomes in full before
    # this is called; the default here is the safe one if it somehow does not.
    _recovery = payload.get("owner_recovery", True)
    _key_made = False
    _key_error = None
    try:
        import profile_keys as _pk
        if r.get("ns"):
            _pk.create_for_profile(r["ns"], (payload.get("password") or ""),
                                   recovery=bool(_recovery))
            _key_made = True
    except Exception as _ke:
        _key_error = str(_ke)
        print(f"[KEYS] could not create a profile key for "
              f"{r.get('username')}: {_ke}", flush=True)

    _audit_api_action(request, "users.create", {
        "username": r.get("username"),
        "profile_key": _key_made,
        "owner_recovery": bool(_recovery) if _key_made else None,
...
async def api_auth_users_delete(request: Request, payload: dict):
    ...
    # The account is gone; its key must go with it. A profile whose data is
    # still readable after deletion is deleted only from the list.
    _key_destroyed = False
    try:
        if r.get("ns"):
            import profile_keys as _pk
            _key_destroyed = _pk.destroy_for_profile(r["ns"])
    except Exception as _ke:
        print(f"[USERS] could not destroy the profile key for {target}: {_ke}",
              flush=True)

    _audit_api_action(request, "users.delete", {
        "username": target,
        "tokens_revoked": revoked,
        "key_destroyed": _key_destroyed,
...
# --- Profile recovery -------------------------------------------------------
# Recovery wraps were cryptographically present from v2.14.3 and had NO path
# that used them, which is a capability that exists only on paper. These are
# that path -- and the place where the difference between the two profile
# types stops being abstract and starts costing somebody their work.

@app.get("/api/auth/users/{username}/recovery")
async def api_profile_recovery_status(username: str, request: Request):
    """Whether the owner can recover this profile. Owner or the user themselves."""
    import session as _session, users as _users
    s = _session.get_session(request.cookies.get(_AUTH_COOKIE))
    if not s:
        raise HTTPException(401, "not signed in")
    target = (username or "").strip()
    if not s.get("is_owner") and (s.get("username") or "").lower() != target.lower():
        raise HTTPException(404)          # not yours, and we do not confirm it exists
    ns = next((u.get("ns") for u in _users.list_users()
               if (u.get("username") or "").lower() == target.lower()), None)
    if not ns:
        raise HTTPException(404, "no such profile")
    try:
        import profile_keys as _pk
        info = _pk.profile_key_info(ns)
        return {"ok": True, "username": target, "has_key": info is not None,
                "recovery_enabled": bool(info and info.get("recovery_enabled"))}
    except Exception as e:
        return {"ok": False, "error": _safe_detail(e, "recovery status")}


@app.post("/api/auth/users/{username}/recovery")
async def api_profile_recovery_set(username: str, payload: dict, request: Request):
    """Turn owner-recovery on or off for a profile.

    OFF may be set by EITHER party alone -- the user claiming sovereignty over
    their own data, or the owner relinquishing access they would rather not
    have. Neither needs the other's agreement.

    ON can only be set by the USER, and not as a matter of policy: granting
    recovery means wrapping the data key for the owner, which requires the
    PLAINTEXT key, which only somebody who can already open the profile has.
    ...
    if not s:
        raise HTTPException(401, "not signed in")
    target = (username or "").strip()
    is_self = (s.get("username") or "").lower() == target.lower()
    if not s.get("is_owner") and not is_self:
        raise HTTPException(404)
    ...
    try:
        import profile_keys as _pk
        if not enable:
            done = _pk.disable_recovery(ns)
            _audit_api_action(request, "profile.recovery_disabled",
                              {"profile": ns, "by": s.get("username")})
            return {"ok": True, "recovery_enabled": False, "changed": bool(done),
                    "note": ("The owner can no longer recover this profile. "
                             "Turning it back on needs this profile's password "
                             "-- nobody can restore it without you.")}

        # Enabling: needs the live data key, so it must be the user's own session.
        if not is_self:
            raise HTTPException(
                403, "only this profile can grant recovery: it requires the "
                     "data key, which only its own password opens")
        dek = _session.get_session_dek(request.cookies.get(_AUTH_COOKIE))
        if not dek:
            raise HTTPException(
                409, "your data key is not open in this session -- sign out and "
                     "back in, then try again")
        _pk.enable_recovery(ns, dek)
        _audit_api_action(request, "profile.recovery_enabled",
                          {"profile": ns, "by": s.get("username")})
        return {"ok": True, "recovery_enabled": True,
                "note": "The owner can now restore access if you lose your password."}
    except HTTPException:
        raise
    except Exception as e:
        return {"ok": False, "error": _safe_detail(e, "recovery set")}


@app.post("/api/auth/users/reset-password")
async def api_owner_reset_password(payload: dict, request: Request):
    """Owner-initiated password reset for a profile.

    Two outcomes, and which one applies was decided when the profile was made:

    RECOVERY ENABLED -- the data key is opened with the machine recovery key
    and re-wrapped under the new password. Nothing is lost.

    RECOVERY NOT ENABLED -- the key cannot be opened by anyone but the person
    who forgot the password, so the existing data is ALREADY unreadable and
    nothing here can change that. The reset is still allowed, because refusing
    only forces a delete-and-recreate that loses the same data with more steps
    -- but it requires {"confirm": "DISCARD DATA"}, destroys the old key rather
    than leaving unreadable files lying about, and mints a fresh one so the
    account works going forward.

    The confirmation string is not decoration. This is the one action in the
    application that can permanently destroy somebody's work, and it should not
    ...
    u = next((x for x in _users.list_users()
              if (x.get("username") or "").lower() == target.lower()), None)
    if not u:
        raise HTTPException(404, "no such profile")
    if u.get("is_owner"):
        raise HTTPException(400, "the owner account cannot be reset this way")
    ns = u.get("ns")

    import profile_keys as _pk
    info = _pk.profile_key_info(ns) if ns else None
    has_recovery = bool(info and info.get("recovery_enabled"))

    if info and not has_recovery and payload.get("confirm") != "DISCARD DATA":
        # Refused, with the consequence stated rather than implied.
        return {
            "ok": False, "needs_confirmation": True, "data_will_be_lost": True,
            "error": (
                "This profile was created so that ONLY its own password can "
                "open it. Its data cannot be recovered by you, by this "
                "application, or by anyone else -- that was the point of the "
                "setting.\n\n"
                "Resetting the password will let %s sign in again, but "
                "EVERYTHING ALREADY STORED WILL BE PERMANENTLY UNREADABLE: "
                "conversations, archives, research, learned procedures.\n\n"
                "There is no undo, and no support path that recovers it "
                "afterwards. If the password might still be remembered, stop "
                "and try that first.\n\n"
                "To proceed anyway, confirm with exactly: DISCARD DATA"
            ) % target,
        }

    try:
        if has_recovery:
            dek = _pk.unlock_with_recovery(ns)
            r = _users.set_password(target, new)
            if not r.get("success"):
                raise HTTPException(400, r.get("error", "could not set the password"))
            _pk.rewrap_password(ns, dek, new)
            outcome = "recovered"
            note = ("Password reset. Their data was recovered and is intact.")
        else:
            r = _users.set_password(target, new)
            if not r.get("success"):
                raise HTTPException(400, r.get("error", "could not set the password"))
            if info:
                # Destroy the old key rather than leaving files nothing can read.
                _pk.destroy_for_profile(ns)
            if ns:
                _pk.create_for_profile(ns, new, recovery=False)
            outcome = "discarded"
            note = ("Password reset. The previous data is permanently "
                    "unreadable and its key has been destroyed. The profile "
                    "starts fresh.")

        import session as _session
        _session.destroy_user_sessions(target)     # force a clean re-login
        _audit_api_action(request, "users.password_reset",
                          {"profile": ns, "username": target, "outcome": outcome})
        return {"ok": True, "outcome": outcome, "note": note}
    except HTTPException:
        raise
    except Exception as e:
        return {"ok": False, "error": _safe_detail(e, "password reset")}
...
async def ws_chat(websocket: WebSocket):
    # v2.14.2: a caller arriving through _ws_bridge has ALREADY been
    # authenticated -- auth.require_scope verified a bearer token before the
    # endpoint ran, and the namespace below comes from that token's binding
    # rather than from anything the caller sent. The session-cookie guard
    # therefore does not apply to it.
    #
    # This is deliberate and commented because it IS an auth bypass. Until now
    # the bridge "passed" this guard by raising AttributeError out of the
    # handler, which is not the same thing as being allowed through: it broke
    # API chat entirely on multi-user installs. Failing closed by accident is
    # not a design. See _MockWebSocket for the trust chain.
    _preauth = bool(getattr(websocket, "veridian_preauth", False))
    _bridge_ns = getattr(websocket, "veridian_ns", None) if _preauth else None

    if config.get("multiuser_enabled", False) and not _preauth:
        try:
            import session as _session
    ...
    try:
        if _preauth:
            ...
            # token, which is the same shared store the single-user case uses.
            _ws_ns = _bridge_ns
        elif config.get("multiuser_enabled", False):
            import session as _session
            ...
            try:
                ...
                if (_last and _last.get("role") == "user"
                    ...
                    if _img_prompt:
                        ...
                        try:
                            _img = await _generate_image_routed(
                                _img_prompt,
                                downloads_dir=str(_downloads_dir_for_ns(_ws_ns)),
                                ns=_ws_ns)
                        except Exception as _ge:
                            ...
                            if action_type in ("search",
                                ...
                                try:
                                    def _search_mem(c=content):
                                        ...
                                        # ns must travel with the list: the
                                        # folder it is read from has to be the
                                        # same namespace it was listed from.
                                        mem = sage_engine.keyword_search(
                                            c, archives, ns=_ws_ns,
                                        )
                                        if mem:
                                            r = ""
                                            for mr in mem:
                                ...
                                try:
                                    _img = await _generate_image_routed(
                                        _gen_prompt,
                                        downloads_dir=str(_downloads_dir_for_ns(_ws_ns)),
                                        _customs_cleared=True, ns=_ws_ns)
                                except Exception as _ge:
                                    _img = {"success": False, "error": str(_ge)}
                                if _img.get("success"):
                                    result = (
