...
def load_archives(archives_dir: Path, ns=None):
    ...
    for file_path in sorted(archives_dir.glob("archive_*.json")):
        try:
            ...
            if _atrest is not None:
                # SYSTEM TIER by default (ns=None): fatigue detection runs
                # inside the CRAIID pipeline, which is background work with no
                # signed-in user, so it reads the owner's store. Same scope and
                # same known limitation as craiid_author. The ns passthrough is
                # here so a profile-aware caller does not have to reopen this
                # loop to get its own key.
                data = _atrest.load_json_auto(blob, ns=ns)
            else:
