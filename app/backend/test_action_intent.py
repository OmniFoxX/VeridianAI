...
try:
    ...
    # The event body is ENCRYPTED on disk (handoff_guard v1 records carry
    # {"v":1,...,"ct":"<fernet token>"}). Grepping the raw log for the intent
    # string could therefore only pass while the chain was in the clear -- it
    # was asserting the ABSENCE of at-rest encryption without saying so. Assert
    # both halves explicitly: not readable on disk, readable through the reader.
    check("intent is not sitting in the log in the clear",
          "opportunistic" not in blob, blob[:200])

    def _detail(rec):
        try:
            return json.loads(rec.get("detail") or "{}")
        except Exception:
            return {"_unparsed": str(rec.get("detail"))[:80]}

    recs = cd._guard.read_audit(decrypt=True) if cd._guard else []
    check("chain event carries intent field",
          bool(recs) and _detail(recs[-1]).get("intent") == "opportunistic",
          str([_detail(r) for r in recs])[:200])
    r2 = cd.inspect("browse", {"url": "https://example.com"}, "mcp")
    recs = cd._guard.read_audit(decrypt=True) if cd._guard else []
    intents = [_detail(r).get("intent") for r in recs]
    check("intent omitted when not supplied (MCP baseline unchanged)",
          intents.count("opportunistic") == 1 and intents[-1] is None,
          str(intents))
