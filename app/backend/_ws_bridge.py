...
class _MockWebSocket:
    ...
    Other attributes are NOT mocked -- if ws_chat ever starts using
    them, the bridge should fail loudly and we update the mock to
    match. That's intentional: silent coverage gaps are worse than
    visible breakage.

    v2.14.2 -- IT DID START USING THEM, and "loudly" turned out to be
    optimistic. When `multiuser_enabled` is True, ws_chat's opening guard
    reads `websocket.cookies` (AttributeError), the `except` calls
    `websocket.close()` (AttributeError again, unhandled), and the whole
    handler raises. Every token-gated endpoint -- /v1/chat/completions,
    /v1/models -- silently stopped working on multi-user installs. It failed
    CLOSED, so nothing leaked, but Continue.dev and Claude Desktop simply
    broke and the bridge reported no events rather than an error.

    So `cookies` and `close` are mocked now, plus the two attributes that
    carry the caller's identity:

      veridian_preauth : True -- this caller was authenticated by BEARER
                         TOKEN before the endpoint ran (auth.require_scope),
                         so the session-cookie guard does not apply.
      veridian_ns      : the namespace that token is bound to.

    TRUST CHAIN, because these attributes are effectively an auth bypass and
    should be readable as one:

        require_scope() verifies the bearer token
          -> endpoint resolves ns from the VERIFIED principal (never from
             the request payload)
          -> run_full_pipeline(ns=...)
          -> _MockWebSocket(bridge_ns=...)
          -> ws_chat trusts it

    A real starlette WebSocket never carries these attributes, and this class
    is only ever constructed inside run_full_pipeline. If either of those
    stops being true, the bypass stops being safe.
    """
    ...
    def __init__(
        ...
        # Present so the cookie guard can read it without raising. Empty
        # because a bridge caller does not authenticate by cookie.
        self.cookies: dict = {}
        self.veridian_preauth = True
        self.veridian_ns = bridge_ns
    ...
    async def close(self, code: int = 1000) -> None:
        """ws_chat closes the socket on a policy refusal. Record the code and
        end the turn, so the bridge returns promptly instead of waiting out
        handler_timeout_sec for events that will never arrive."""
...
async def run_full_pipeline(
    ...
    *,
    ...
    ns            : namespace this turn belongs to. None = owner / shared
                    store, which is also the single-user case. MUST come from
                    a verified API token, never from caller-supplied data --
                    it bypasses the session check. See _MockWebSocket.
