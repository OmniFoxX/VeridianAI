...
if TAVILY_KEY_FILE.exists():
    ...
    try:
        ...
        # SYSTEM TIER, deliberately no ns: a machine-level integration secret,
        # not any profile's data. Encrypting it per-profile would make it
        # unreadable to whichever profile did not create it.
...
def _load_titles(ns=None) -> dict:
    ...
    if not p.exists():
        return {}
    try:
        d = atrest.load_json_auto(p.read_bytes(), ns=ns)
...
def _save_titles(titles: dict, ns=None):
    (_archive_folder(ns) / _TITLES_FILE).write_bytes(
        atrest.dump_json_encrypted(titles, ns=ns))
...
def _suggest_fork_title(history: list, ns=None, *, max_scan: int = 40):
    ...
    try:
        ...
        for f in files[:max_scan]:
            try:
                other = atrest.load_json_auto(f.read_bytes(), ns=ns)
            except Exception:
                continue
...
def archive_conversation(history: list, ns=None) -> dict:
    ...
    try:
        ...
        path.write_bytes(atrest.dump_json_encrypted(history, ns=ns))
...
def get_archives(ns=None) -> list:
    ...
    for f in sorted(folder.iterdir(), reverse=True):
        if f.name.startswith("archive_") and f.suffix == ".json":
            try:
                data = atrest.load_json_auto(f.read_bytes(), ns=ns)
                archives.append({
                    "filename": f.name,
...
def load_archive(filename: str, ns=None) -> dict:
    ...
    try:
        with open(target, "rb") as _f:
            data = atrest.load_json_auto(_f.read(), ns=ns)
...
def keyword_search(query: str, archives: list, ns=None) -> list:
    """Keyword scan over a caller's archives.

    v2.14.3 -- CROSS-PROFILE LEAK, fixed here. This read every file from the
    module-level ARCHIVE_FOLDER, which is the OWNER'S folder, while the
    `archives` list it was given came from get_archives(ns) -- somebody else's.
    Two ways to be wrong, both silent:

      * filenames are timestamps, so a collision between two profiles was
        entirely possible -- and then a non-owner searching their own archives
        was served the OWNER'S content, scored and returned as their own
      * with no collision the open simply failed, `continue` swallowed it, and
        that profile's search quietly returned nothing at all

    Demonstrated with a deliberate collision on 2026-08-14: alice searching for
    a word present only in the owner's archive got the owner's medical note.

    The archives list and the folder they are read from must come from the SAME
    namespace. Passing the list without the namespace it was built from is what
    ...
    for a in archives:
        try:
            text = " ".join(m.get("content", "").lower() for m in
                            atrest.load_json_auto((folder / a["filename"]).read_bytes(),
                                                  ns=ns))
...
def semantic_search(query: str, archives: list, ollama_url: str = "http://localhost:11434",
                    ns=None) -> list:
    """Semantic search via sage_rag vector pipeline. Delegates entirely to sage_rag.py.

    v2.14.3: same folder fix as keyword_search -- paths were built from the
    OWNER'S ARCHIVE_FOLDER regardless of whose archives were being searched.

    The vector INDEX is now per-profile too. A shared index holds embeddings
    derived from archive text, so one index across profiles means one profile's
    query can be scored against another's content -- the same leak one layer
    further in, and harder to notice because the output is a similarity score
    ...
    try:
        ...
        index_path = str((base / "vector_index.json") if base
                         else (DATA_DIR / "vector_index.json"))
        folder = _archive_folder(ns)
        # archives from get_archives() are dicts -- extract full paths
        fnames = []
        for a in archives:
            if isinstance(a, dict):
                fnames.append(str(folder / a["filename"]))
            else:
                fnames.append(str(a))
        return sage_rag.semantic_search(query, fnames, index_path,
                                        ollama_url, ns=ns)
    except Exception:
        return []


def search_all_archives(query: str, ns=None) -> list:
    """Combined keyword + semantic search, deduplicated.

    v2.14.2: takes `ns` like its sibling get_archives(ns) already did. It was
    the only archive reader that could not be scoped, which is how the MCP
    `search_memory` tool ended up reading the OWNER's archives no matter whose
    API token asked. None = owner / shared store, as everywhere else.
    """
    archives = get_archives(ns)[:25]
    # The ns MUST be passed on: the list and the folder it is read from have to
    # be the same namespace. See keyword_search for what happened when they
    # were allowed to differ.
    keyword_results = keyword_search(query, archives, ns=ns)
    semantic_results = semantic_search(query, archives, ns=ns)
    combined = []
    seen = set()
...
def _browser_persist_cookies(ns=None) -> bool:
    """Opt-in cookie persistence; default OFF (cookies cleared each session).
    Reads env OAI_BROWSER_PERSIST_COOKIES, then this profile's ui_prefs.json
    (browser_persist_cookies, set by the Settings toggle), else False.

    ns matters: the toggle is per profile, so browsing on behalf of one person
    must not inherit another's choice about whether their logins are kept."""
    try:
        import os as _os
    ...
    try:
        ...
        return bool(_uip.get("browser_persist_cookies", False, ns=ns))
    except Exception:
        pass
    return False
...
def _get_browser():
    ...
    try:
        ...
        return _run_browser_coro(
            get_browser(ns=_ns,
                        persist_cookies=_browser_persist_cookies(_ns)),
            timeout=28000.0,
        )
    except Exception as e:
...
def downloads_dir_for(ns=None):
    """The downloads folder for a namespace. None -> the shared/owner folder.

    Mirrors main._downloads_dir_for_ns, which already did this for generated
    images; saved FILES were the one path still writing everyone into the
    owner's folder.
    """
    base = user_data_dir(ns)
    if not base:
    ...
    try:
        ...
        if path.exists():
            try:
                ...
                _sh.copy2(path, _dl / backup_name)
                baks = sorted(_dl.glob(f"{safe_name}.*.bak"))
                for old in baks[:-3]:
                    try:
                        old.unlink()
                    except OSError:
...
def set_tavily_key(key: str) -> dict:
    ...
    try:
        ...
        import atrest as _atrest
        # SYSTEM TIER, deliberately no ns -- see the read at module level.
