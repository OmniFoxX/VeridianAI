...
class HandoffGuard:
    ...
    def audit(self, event: str, detail: str = "") -> None:
        ...
        try:
            ...
            # SYSTEM TIER: CRAIID handoff records are pipeline state, not
            # any profile's content, and are written by background work
            # that has no signed-in user. See craiid_author for the scope.
                ...
                for i, line in enumerate(f, 1):
                    ...
                    if rec.get("v") == 1 or "ct" in rec:
                        ...
                        if decrypt:
                            try:
                                import atrest
                                # SYSTEM TIER: the audit chain is written by
                                # background work and must stay readable when
                                # no profile is unlocked -- see audit().
            ...
            with open(self.audit_log, "r", encoding="utf-8") as f:
                for line in f:
                    ...
                    if decrypt and (rec.get("v") == 1 or "ct" in rec):
                        ...
                        try:
                            import atrest
                            # SYSTEM TIER: reads back what audit() wrote under
                            # the system key. A profile key here would make the
                            # chain unverifiable while that profile is logged
                            # out, which defeats the point of a chain.
