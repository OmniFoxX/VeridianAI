...
# The atrest stub is KEYED BY NAMESPACE on purpose. A stub that accepted ns
# and ignored it would let a caller that forgot to pass ns read another
# profile's ledger, and this file would report that as a pass -- which is
# exactly how the cross-profile archive leak survived its own test suite.
_AT_CALLS = []                       # (op, ns) for each call the ledger makes


def _stub_encrypt(o, ns=None):
    _AT_CALLS.append(("enc", ns))
    return b"ENC" + json.dumps({"ns": ns, "d": o}).encode()


def _stub_decrypt(b, ns=None):
    _AT_CALLS.append(("dec", ns))
    if b[:3] != b"ENC":
        return json.loads(b)
    box = json.loads(b[3:].decode())
    if box.get("ns") != ns:           # stands in for Fernet's InvalidToken
        raise ValueError("sealed for ns=%r, opened with ns=%r" % (box.get("ns"), ns))
    return box["d"]


_at = types.ModuleType("atrest")
_at.dump_json_encrypted = _stub_encrypt
_at.load_json_auto = _stub_decrypt
...
# The ledger must reach atrest WITH the namespace, not merely write to the
# right folder. Right folder + owner's key is still a cross-profile read.
check("every at-rest op carried ns=alice",
      bool(_alice_ops) and all(ns == "alice" for _op, ns in _alice_ops),
      str(_alice_ops))
_sealed = (_TMP / "users" / "alice" / ".evidence_ledger.dat").read_bytes()
try:
    _stub_decrypt(_sealed, ns=None)
    check("alice's ledger is unreadable with the owner's key", False)
except ValueError:
    check("alice's ledger is unreadable with the owner's key", True)
...
_at.dump_json_encrypted = lambda o, ns=None: (_ for _ in ()).throw(RuntimeError("no key"))
