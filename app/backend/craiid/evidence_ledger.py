...
def _warn(msg: str) -> None:
    ...
    Every path below returns a degraded-but-safe result (an empty ledger, or a
    refusal to write). That is correct for the caller and useless to a
    maintainer, so the reason goes to stderr on its way out.
    """
    try:
        sys.stderr.write("[evidence_ledger] " + str(msg) + "\n")
    except Exception:
        pass
...
def _load(ns=None) -> Dict:
    ...
    try:
        ...
        # An unreadable ledger is not nothing: this thread's evidence is gone
        # and the model will be told there are no particulars. Degrading
        # quietly to "no sources found" is the exact failure this module
...
def _save(data: Dict, ns=None) -> bool:
    ...
    try:
        ...
        try:
            ...
            return False
        try:
            blob = atrest.dump_json_encrypted(data, ns=ns)
        except Exception as exc:
            # NOT the same case as "atrest is absent". Reaching here means the
            # encrypt call itself failed -- a bad key, or signature drift
            # between this caller and atrest. Both are defects, and a bare
            # False reports them as "encryption unavailable". Still refuse to
            # write plaintext; just do not lie about why.
            _warn("encrypt failed (%s: %s)" % (type(exc).__name__, exc))
            return False
        p.write_bytes(blob)
        return True
    except Exception as exc:
