...
class ProceduralMemory:
    ...
    def __init__(self,
        ...
        Args:
            ...
            owner_ns:      Which profile this store belongs to. None = the
                           owner / single-user shared store.

        v2.14.2 -- WHY THE STORE IS PER-PROFILE BUT THE CHAIN IS NOT.
        ...
        The hash chain stays SHARED, deliberately. It is the audit log, and an
        audit log that records every actor in one tamper-evident sequence is
        both the point and the stronger integrity property: one head to verify,
        one continuous record. Splitting it per user would turn "has anything
        been tampered with?" from one question into N, and would hide each
        user's activity from the owner -- the opposite of what an audit
        control is for.

        So entries are ATTRIBUTED instead of separated: owner_ns is stamped
        into the chain metadata of every witness this store writes.
        """
        self.owner_ns = owner_ns
    ...
    def _witness_to_chain(self, key: str, value: Any,
        ...
        meta.update({
            ...
            # Attribution, so the shared chain can answer WHO as well as WHAT.
            "owner_ns": self.owner_ns or "(owner)",
        })
