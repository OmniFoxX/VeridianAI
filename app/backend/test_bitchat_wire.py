...
if not os.path.exists(PATH):
    # BitChat was unregistered as a Socials channel when Argo-Net replaced it,
    # and bitchat-python is an EXTERNAL sibling checkout, not part of this
    # repo. Missing it is expected here; a hard FileNotFoundError in the suite
    # reads like a regression and trains the eye to ignore red.
    print("SKIP test_bitchat_wire: bitchat-python not present at " + PATH)
    print("     (external checkout; the wire-ID fix still needs a live phone test)")
    raise SystemExit(0)
