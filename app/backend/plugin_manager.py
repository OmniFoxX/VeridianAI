...
class PluginManager:
    ...
    def postprocess(self, response: str) -> str:
        ...
        will start writing it itself. Appending unconditionally then yields the
        disclosure twice, which reads as a bug in the very line that exists to
        be trusted.

        Checking the tail also covers the plainer case: two code paths that
        both call postprocess on the same string.
        """
        for plugin in self._plugins.values():
            if not plugin.get("enabled", True):
                continue
            ...
            if not footer:
                continue
            # Compare on stripped text: the stored hook carries leading
            # newlines and a rule, and a model's imitation will not reproduce
            # that whitespace exactly.
            if response.rstrip().endswith(str(footer).strip()):
                continue
            response += "\n" + footer
        return response
