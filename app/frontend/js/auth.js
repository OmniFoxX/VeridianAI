...
(function () {
  ...
  function showUserAdmin() {
    ...
    ov.innerHTML =
      ...
      // Recovery choice. BOTH outcomes are stated, not one with a footnote:
      // the second option can permanently destroy someone's work later, and a
      // person choosing it should have read what that means at the moment they
      // chose it -- not discovered it the day the password is forgotten.
      '<fieldset style="border:1px solid ' + V("--border", "#2a3a5a") +
      ';border-radius:8px;padding:8px 10px;margin:8px 0 4px">' +
      '<legend style="font-size:12px;padding:0 4px;color:' +
      V("--text-muted", "#7890b8") + '">If they forget their password</legend>' +

      '<label style="display:flex;gap:7px;font-size:12px;cursor:pointer;margin-bottom:6px;color:' +
      V("--text", "#e2e8f8") + '">' +
      '<input type="radio" name="ua-recovery" id="ua-rec-yes" value="1" checked ' +
      'style="accent-color:' + V("--gold", "#f0a500") + ';margin-top:2px">' +
      '<span><strong>You can restore their access.</strong><br>' +
      '<span style="color:' + V("--text-muted", "#7890b8") + '">Their data is safe if the ' +
      'password is lost. It also means you are able to read this profile.</span></span></label>' +

      '<label style="display:flex;gap:7px;font-size:12px;cursor:pointer;color:' +
      V("--text", "#e2e8f8") + '">' +
      '<input type="radio" name="ua-recovery" id="ua-rec-no" value="0" ' +
      'style="accent-color:' + V("--gold", "#f0a500") + ';margin-top:2px">' +
      '<span><strong>Only their password opens it.</strong><br>' +
      '<span style="color:' + V("--text-muted", "#7890b8") + '">Nobody can read it without ' +
      'them &mdash; not you, not this app. <strong>If the password is lost, the ' +
      'data is gone permanently and no one can get it back.</strong></span></span></label>' +
      '</fieldset>' +
  ...
  async function uaLoad() {
    ...
    try {
      ...
      box.innerHTML = users.map(function (u) {
        ...
        var actions = u.is_owner
          ...
          : '<button type="button" class="ua-access" data-u="' + encodeURIComponent(u.username) +
            ...
            '<button type="button" class="ua-pw" data-u="' + encodeURIComponent(u.username) +
            '" style="' + btnStyle + 'color:' + V("--text-muted", "#7890b8") + ';margin-right:6px">Reset password</button>' +
      ...
      box.querySelectorAll(".ua-pw").forEach(function (b) {
        b.onclick = function () { uaResetPassword(decodeURIComponent(b.getAttribute("data-u"))); };
      });
      // Which profiles the owner can actually recover. Fetched per profile so
      // the badge reflects the KEY, not a setting somebody could have changed
      // without the key following.
      users.forEach(function (u) {
        if (u.is_owner) return;
        fetch("/api/auth/users/" + encodeURIComponent(u.username) + "/recovery",
              { credentials: "same-origin" })
          .then(function (r) { return r.json(); })
          .then(function (d) {
            if (!d || !d.ok || !d.has_key || d.recovery_enabled) return;
            var rows = box.querySelectorAll("div");
            for (var i = 0; i < rows.length; i++) {
              if (rows[i].textContent.indexOf(u.username) === 0) {
                var tagEl = document.createElement("span");
                tagEl.style.cssText = "font-size:10px;padding:1px 6px;border-radius:8px;" +
                  "margin-left:6px;background:" + V("--surface-3", "#142036") +
                  ";color:" + V("--text-muted", "#7890b8");
                tagEl.textContent = "password-only";
                tagEl.setAttribute("data-tip",
                  "Only this profile's password can open its data. You cannot " +
                  "recover it, and neither can anyone else.");
                rows[i].querySelector("span").appendChild(tagEl);
                break;
              }
            }
          })
  ...
  async function uaCreate() {
    ...
    try {
      ...
      // Default TRUE if the control is somehow missing: the recoverable option
      // is the one that cannot lose data.
      var recNo = document.getElementById("ua-rec-no");
      var ownerRecovery = !(recNo && recNo.checked);
      var r = await fetch("/api/auth/users", {
        method: "POST", headers: { "Content-Type": "application/json" }, credentials: "same-origin",
        body: JSON.stringify({ username: u, password: p, restricted: restricted,
                               owner_recovery: ownerRecovery }),
      });
  ...
  //
  // Two outcomes, decided when the profile was created. The recoverable case
  // is unremarkable. The other one is the single action in this application
  // that can permanently destroy somebody's work, so it is deliberately
  // awkward: the consequence is spelled out in full, then typed back.
  async function uaResetPassword(username) {
    var pw = (typeof window.oraclePrompt === "function")
      ? await window.oraclePrompt(
          "New password for " + username + ":",
          { title: "Reset password", okLabel: "Continue" })
      : window.prompt("New password for " + username + ":");
    if (!pw) return;

    async function send(confirm) {
      var body = { username: username, new_password: pw };
      if (confirm) body.confirm = confirm;
      var r = await fetch("/api/auth/users/reset-password", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        credentials: "same-origin",
    ...
    var d;
    try { d = await send(null); }
    catch (e) { uaSetError("Could not reach the server."); return; }

    if (d && d.ok) {
      uaSetError("");
      if (window.setStatus) window.setStatus(d.note || "Password reset.");
      uaLoad();
      return;
    }

    if (!d || !d.needs_confirmation) {
      uaSetError((d && d.error) || "Could not reset the password.");
      return;
    }

    // The destructive path. Stage 1: read it.
    var proceed = await oracleConfirm(
      d.error + "\n\nDo you want to continue?",
      { title: "This will destroy " + username + "'s data",
        okLabel: "I understand, continue", cancelLabel: "Stop" },
    );
    if (!proceed) { uaSetError("Reset cancelled -- nothing was changed."); return; }

    // Stage 2: type it. Same shape as Burn, for the same reason.
    var typed = (typeof window.oraclePrompt === "function")
      ? await window.oraclePrompt(
          "Type DISCARD DATA (exactly, in capitals) to confirm that " +
          username + "'s existing data will be permanently unreadable:",
          { title: "Final confirmation", okLabel: "Discard and reset" })
      : window.prompt("Type DISCARD DATA to confirm:");
    if (typed !== "DISCARD DATA") {
      uaSetError("Reset cancelled -- nothing was changed.");
      return;
    }

    try {
      var d2 = await send("DISCARD DATA");
      if (d2 && d2.ok) {
        uaSetError("");
        if (window.setStatus) window.setStatus(d2.note || "Password reset.");
        uaLoad();
      } else {
        uaSetError((d2 && d2.error) || "Could not reset the password.");
      }
    } catch (e) {
      uaSetError("Could not reach the server.");
    }
  }
