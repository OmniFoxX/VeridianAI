...
(function () {
  ...
  /* The row now lives inside the export/import panel, which is built when the
   * panel opens -- long after DOMContentLoaded. refresh() binds by id and
   * returns quietly when the ids are absent, so the panel calls this once it
   * has created them. */
  window.dataFolderRefresh = refresh;
