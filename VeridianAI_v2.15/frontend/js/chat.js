...
function oracleConfirm(message, opts) {
  ...
  return new Promise(function (resolve) {
    ...
    root.innerHTML =
      ...
      // An optional third button that does NOT answer the question. It runs
      // something and leaves the dialog standing, which is the only useful
      // shape for "let me go and check first" -- closing the dialog to do the
      // errand means coming back and starting the decision over.
      (opts.extraLabel
        ? '<button class="modal-btn" id="oracle-confirm-extra">' +
          escapeHtml(opts.extraLabel) +
          "</button>"
        : "") +
    ...
    var extra = document.getElementById("oracle-confirm-extra");
    if (extra) {
      extra.onclick = function () {
        try {
          if (typeof opts.onExtra === "function") opts.onExtra();
        } catch (e) {
          /* an errand that fails must not answer the question either */
        }
      };
    }
...
async function burnAllData() {
  const ok = await oracleConfirm(
    "This permanently ERASES ALL of your data:\n\n" +
      ...
      "There is no recovery and no backup. If you want to keep any of it,\n" +
      "open your data folder now and take a copy - this dialog will wait.\n\n" +
      "Continue?",
    {
      title: "🔥 Burn all my data",
      okLabel: "Continue…",
      cancelLabel: "Cancel",
      // The last moment anyone can save anything. Offering the door here
      // costs one button; not offering it costs somebody their archives.
      extraLabel: "📁 Open Data Folder",
      onExtra: function () {
        if (typeof window.openDataFolder === "function") window.openDataFolder();
      },
    },
  );
  if (!ok) return;
