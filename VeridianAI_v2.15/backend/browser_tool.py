...
def _channel_profile_dir(root: Path, label: str) -> Path:
    """One profile directory PER BROWSER, under the profile's own root.

    A Chromium user-data directory is not portable across builds. Every channel
    was pointed at the same folder, so whichever browser touched it last left
    it in a state the next one rejects -- observed as an immediate exit
    (exitCode=21) and "Settings version is not 7" from Edge. The fallback chain
    then made it look healthy, because bundled Chromium eventually succeeded:
    the first two rungs could NEVER succeed, which is precisely why the
    real-browser identity (the user's own logins and cookies) never engaged.

    Separate directories also mean a Brave session and a Chrome session no
    longer inherit each other's cookies, which is the behaviour anyone would
    assume from picking a browser.
    """
    safe = "".join(c if (c.isalnum() or c in "-_") else "_" for c in (label or "default"))
    return root / ("browser_profile_" + (safe or "default"))
...
class BrowserTool:
    ...
    async def start(self) -> None:
        ...
        base_kwargs = dict(
            # user_data_dir is filled in per attempt below -- see
            # _channel_profile_dir. Left out here on purpose so a future edit
            # cannot reintroduce one shared directory by forgetting to override.
            user_data_dir=None,
        ...
        for label, overrides in attempts:
            ...
            try:
                prof = _channel_profile_dir(self.profile_dir, label)
                prof.mkdir(parents=True, exist_ok=True)
                self.context = await self.playwright.chromium.launch_persistent_context(
                    **{**base_kwargs, **overrides, "user_data_dir": str(prof)}
                )
                self.active_profile_dir = prof
