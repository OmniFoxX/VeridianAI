...
async def relay_request(payload: dict, request: Request):
    _gate(request)
    # RelayHub's methods are coroutines (they take an asyncio.Lock). Without
    # the await nothing is ever enqueued and the caller still gets a
    # request_id-shaped 500 -- the whole relay HTTP layer was inert.
    rid = await _hub.submit_request(str(payload.get("target", "")),
                                    payload.get("payload"))
    if rid is None:
...
async def relay_poll(peer_id: str, request: Request):
    _gate(request)
    return (await _hub.next_request(peer_id)) or {}


@relay_router.post("/respond")
async def relay_respond(payload: dict, request: Request):
    _gate(request)
    await _hub.submit_response(str(payload.get("request_id", "")),
...
async def relay_response(request_id: str, request: Request):
    _gate(request)
    return await _hub.get_response(request_id)
