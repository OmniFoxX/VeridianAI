...
# APP AND PROCESS STATE stays with the install. Settings, the pid registry
# and the lock directory describe THIS installation, not the person using it.
CONFIG_FILE      = STATE_DIR / "config.json"
PID_REGISTRY     = STATE_DIR / ".oracle_pids.json"

# USER CONTENT lives in sage_data, with everything else that belongs to a
# person (2026-08-13).
#
# These four followed STATE_DIR, which is the INSTALL directory whenever it is
# writable. Three problems, in order of how much they cost:
#
#   1. The portable build upgrades by unzipping a new version over the old
#      one. Conversations sitting in the install directory are standing
#      exactly where the new version lands.
#   2. Every profile's data was already under DATA_DIR. The owner was the one
#      person whose conversations lived somewhere else -- and a rule that
#      holds for everyone except one account is the shape that produced the
#      export containment leak.
#   3. The export manifest tells people to copy their folders "into the new
#      install's sage_data directory". For owner chat and archives that
#      instruction was simply wrong.
#
# The OLD_ names are kept so the one-time move can find what it is moving.
# See state_migration.py; nothing else should read them.
_USER_DATA_DIR   = data_dir()
CHAT_MEMORY_FILE = _USER_DATA_DIR / "chat_memory.json"
ARCHIVES_DIR     = _USER_DATA_DIR / "archives"
UPLOADS_DIR      = _USER_DATA_DIR / "uploads"
DOWNLOADS_DIR    = _USER_DATA_DIR / "downloads"

OLD_CHAT_MEMORY_FILE = STATE_DIR / "chat_memory.json"
OLD_ARCHIVES_DIR     = STATE_DIR / "archives"
OLD_UPLOADS_DIR      = STATE_DIR / "uploads"
OLD_DOWNLOADS_DIR    = STATE_DIR / "downloads"
