...
# v2.14.3 -- CRAIID reads the SHARED (owner) store, not a profile.
#
# Not an oversight and not a leak: sage_daemon, the overseer and this
# author all run as background work with no signed-in user, so there is
# no namespace to be scoped to and no profile key available to open. The
# warm-handoff pipeline therefore operates on the owner's store only.
#
# KNOWN LIMITATION, not silently accepted: a non-owner profile gets no
# CRAIID warm handoff. Making it per-profile means giving the daemon a
# notion of whose turn it is processing and a way to hold that key --
# a larger piece of work than the encryption switchover, and one that
# should be decided rather than drifted into.
def _load_archives(
    ...
    for af in archive_files:
        try:
            ...
            blob = af.read_bytes()
            # SYSTEM TIER (owner scope): the CRAIID author runs as background
            # work with no signed-in user, so it reads the owner's store.
            # KNOWN LIMITATION, recorded deliberately: warm handoff does not
            # yet cover non-owner profiles. Adding ns here without a way to
            # obtain that profile's key would fail closed at every read.
        ...
        for chunk_file in sorted(_VLTS_DIR.glob("chunk_*.json")):
            try:
                if _atrest is not None:
                    ...
                    # SYSTEM TIER (owner scope) -- same reason as the archive
                    # loader above.
    ...
    try:
        if _atrest is not None:
            with os.fdopen(tmp_fd, "wb") as f:
                # SYSTEM TIER (owner scope): the warm-instance reconstruct is
                # pipeline state written by background work. It must be
                # readable by the next daemon start, when nobody is logged in.
