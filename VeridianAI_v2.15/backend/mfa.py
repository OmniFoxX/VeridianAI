...
def _load():
    ...
    try:
        with open(p, "rb") as f:
            blob = f.read()
        try:
            # SYSTEM TIER, deliberately no ns. This store is read during
            # LOGIN to verify a second factor -- before anyone is
            # authenticated, and therefore before any profile key exists.
            # Encrypting it per-profile would make signing in impossible.
            import atrest
...
def _save(store):
    p = _store_path()
    try:
        # SYSTEM TIER -- see _load. One store for every user, of necessity.
        import atrest
...
# v2.14.3 -- a profile's data key, held across the second-factor hop.
#
# The plaintext password exists only at the FIRST login step; the session is
# minted at the SECOND. Without carrying the unwrapped key across, every
# MFA-enabled profile would sign in successfully and then be unable to read its
# own data -- a failure that would look like corruption rather than a missing
# hop.
#
# Separate map, not a field on the pending record, for the same reason
# session._DEKS is separate: consume_challenge() RETURNS that record, and
# returned records get logged.
_PENDING_DEKS = {}


def stash_challenge_dek(token, dek):
    """Park an unwrapped data key against a pending second-factor challenge."""
    if not token or not dek:
        return False
    with _PLOCK:
        if token not in _PENDING:
            return False
        _PENDING_DEKS[token] = dek
    return True


def take_challenge_dek(token):
    """Pop the key for a challenge. One-shot, like the challenge itself."""
    if not token:
        return None
    with _PLOCK:
        return _PENDING_DEKS.pop(token, None)
...
def begin_challenge(user, must_change=False, ttl=None):
    ...
    with _PLOCK:
        ...
        for t in [t for t, r in _PENDING.items() if r["expires"] < now]:
            _PENDING.pop(t, None)
            _PENDING_DEKS.pop(t, None)      # abandoned challenge -> key goes too
        # ORPHANS: a challenge that was consumed but whose key was never
        # collected leaves a DEK with no pending record, and the sweep above
        # only sees tokens still IN _PENDING -- so it would live for the life
        # of the process. Key material must not outlive its purpose.
        for t in [t for t in _PENDING_DEKS if t not in _PENDING]:
            _PENDING_DEKS.pop(t, None)
...
def consume_challenge(token):
    ...
    with _PLOCK:
        ...
        if not r or r["expires"] < time.time():
            _PENDING_DEKS.pop(token or "", None)
            return None
        return r
